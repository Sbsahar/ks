...
def start_attack(call):
    ...
    if not player_row:
        ...
        return
    level, troops_str, clan_name = player_row
    if json.loads(troops_str) == {}:  # إصلاح: التحقق من وجود جيش
        bot.answer_callback_query(call.id, "ليس لديك جيش كافٍ للهجوم! درب جنود أولاً. 💂", show_alert=True)
        return
        
    # الآن ابدأ المعركة (الكود الذي أزلناه سابقاً)
    ...
    if clan_name:
        ...
        if clan_row:
            ...
            if clan_level >= 5:
                # +1 مستوى للجنود الدعم (يمكنك تنفيذ الزيادة هنا إذا لزم)
                pass
    ...
    def battle_thread():
        ...
        for minute in range(5):
            ...
            bot.send_message(call.message.chat.id, f"دقيقة **{minute+1}**: دمرت **{destruction}%** من الخصم! 🔥💥", parse_mode='Markdown')
            time.sleep(0.1)
        ...
        bot.send_message(call.message.chat.id, f"انتهت المعركة! نجوم: **{stars}** ⭐✨\nكؤوس مكتسبة: **{trophies_won}** 🏆\nغنائم: **{loot}** ذهب/إكسير. 🎁", parse_mode='Markdown')
        time.sleep(0.1)

        # إشعار للخصم
        try:
            if destruction >= 50:
                bot.send_message(opp_id, f"🛑 **تعرضت للهجوم من {call.from_user.username}!** خسرت **{trophies_won}** كؤوس و**{loot}** موارد. 😔", parse_mode='Markdown')
            else:
...
def battle_actions(call):
    ...
    if data[0] == 'withdraw':
        ...
        # جلب بيانات اللاعب والخصم مرة أخرى (للتأكيد)
        c.execute("SELECT level, troops FROM players WHERE id=?", (user_id,))
        player_row = c.fetchone()
        if not player_row:
            bot.answer_callback_query(call.id, "حدث خطأ في بياناتك!")
            return
        level, troops_str = player_row  # إصلاح: جلب troops_str هنا
        destruction = int(data[3])  # destruction الحالي
