...
battle_status = {}  # قاموس لتتبع المعركة
...
c.execute('''CREATE TABLE IF NOT EXISTS clans 
             (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE, level INT DEFAULT 1, resources TEXT DEFAULT '{}', members TEXT DEFAULT '[]', 
...
def show_opponent_info(chat_id, opp_id, opp_username, opp_level, opp_troops, opp_gold, opp_elixir, user_id, is_revenge=False):
    ...
    bot.send_message(
        ...
        f"💂 <b>جنود:</b> {', '.join(f'{k}: {v}' for k, v in troops.items())}\n"
...
def start_battle(message):
    ...
    user_id = message.from_user.id
    c.execute("SELECT troops FROM players WHERE id=?", (user_id,))
    troops_row = c.fetchone()
    if not troops_row or json.loads(troops_row[0]) == {}:
        bot.reply_to(message, "ليس لديك جيش كافٍ للهجوم! درب جنود أولاً. 💂", parse_mode='Markdown')  # إصلاح: تحقق مبدئي على الجيش
        time.sleep(0.1)
        return
...
def transfer(message):
    ...
    user_id = message.from_user.id
    c.execute("SELECT level, gold, elixir, clan_name FROM players WHERE id=?", (user_id,))
    row = c.fetchone()
    if not row or not row[3]:
        bot.reply_to(message, "أنت غير منضم إلى تحالف! انضم أولاً باستخدام 'انضم تحالف [اسم]'. 👥", parse_mode='Markdown')
        time.sleep(0.1)
        return
    clan_name = row[3]
    ...
    if not target_row or target_row[0] != clan_name:
    ...
    clan_row = c.fetchone()
    if not clan_row:
        bot.reply_to(message, "التحالف غير موجود في قاعدة البيانات! ⚠️", parse_mode='Markdown')
        time.sleep(0.1)
        return
    clan_level = clan_row[0]

    field = 'gold' if resource_type == 'ذهب' else 'elixir'
    c.execute(f"SELECT {field} FROM players WHERE id=?", (user_id,))
    user_resources = c.fetchone()
    if not user_resources or user_resources[0] < amount:
        bot.reply_to(message, "موارد غير كافية! 💸", parse_mode='Markdown')
        time.sleep(0.1)
        return
...
def start_attack(call):
    ...
    if user_id != call.from_user.id:
        ...
        return
    c.execute("SELECT username, level, troops, gold, elixir FROM players WHERE id=?", (opp_id,))
    opponent_row = c.fetchone()
    if not opponent_row:
        bot.answer_callback_query(call.id, "الخصم غير متاح الآن!", show_alert=True)
        return
    opp_username, opp_level, opp_troops, opp_gold, opp_elixir = opponent_row
        
    c.execute("SELECT level, troops, clan_name, trophies FROM players WHERE id=?", (user_id,))
    player_row = c.fetchone()
    if not player_row:
        bot.answer_callback_query(call.id, "حدث خطأ في بياناتك!", show_alert=True)
        return
    level, troops_str, clan_name, current_trophies = player_row
    if json.loads(troops_str) == {}:
        bot.answer_callback_query(call.id, "ليس لديك جيش كافٍ للهجوم! درب جنود أولاً. 💂", show_alert=True)
        return
        
    # تحديث حالة المعركة
    battle_status[user_id] = {'running': True, 'message_id': call.message.message_id, 'chat_id': call.message.chat.id}
    
    bot.edit_message_text(f"بدأت معركة ضد **{opp_username}** مستوى **{opp_level}**! ⚔️💥\nمدة: 5 دقائق. انتظر التحديثات.", 
    ...
    if clan_name:
        ...
        if clan_row:
            ...
            clan_level = clan_row[1]
            if clan_level >= 5:
                support_boost = 1 + 0.1 * (clan_level - 4)
                for troop in support_troops:
                    support_troops[troop] = int(support_troops[troop] * support_boost)

    def battle_thread():
        attacker_power = calculate_army_power(troops_str, level)
        defender_power = calculate_army_power(opp_troops, opp_level)
        defender_hp = calculate_village_hp(opp_troops, opp_level)
        destruction = 0
        for minute in range(5):
            if not battle_status.get(user_id, {}).get('running', False):  # إيقاف إذا انسحب
                return
            ...
            effective_damage = max(1, damage * 0.5 - defender_power * random.uniform(0.3, 0.7))  # إصلاح: ضمان قيمة إيجابية صغيرة
            defender_hp -= effective_damage
            if defender_hp <= 0:
                destruction = 100
            else:
                destruction = min(100, destruction + int((effective_damage / calculate_village_hp(opp_troops, opp_level)) * 100 * random.uniform(0.9, 1.1)))
            markup = InlineKeyboardMarkup()
            if destruction < 100 and battle_status.get(user_id, {}).get('running', False):
                markup.add(InlineKeyboardButton("انسحاب 🏃", callback_data=f"withdraw:{user_id}:{opp_id}:{destruction}"))
            bot.send_message(call.message.chat.id, f"🕒 دقيقة **{minute+1}**: دمرت **{destruction}%** من الخصم! 🔥💥", reply_markup=markup, parse_mode='Markdown')
            if destruction >= 100:
                bot.send_message(call.message.chat.id, "🏆 انتصرت! تدمير كامل 100%! 🎉", parse_mode='Markdown')
                break
        ...
        if stars == 0:
            trophies_change = -10
            if current_trophies < 10:
                trophies_change = -current_trophies
        else:
            trophies_change = stars * (trophy_offer // 3)

        loot_percent = 0.1 + 0.05 * stars
        loot = int(loot_percent * 1000 * level)

        # خسارة الجنود بعد المعركة
        loss_percent = 0.5 if stars < 2 else 0.3
        new_troops = lose_troops(troops_str, loss_percent)
        c.execute("UPDATE players SET troops = ? WHERE id=?", (new_troops, user_id))

        c.execute("UPDATE players SET trophies = trophies + ?, gold = gold + ?, elixir = elixir + ? WHERE id=?", (trophies_change, loot, loot, user_id))
        c.execute("UPDATE players SET trophies = trophies - ? WHERE id=?", (trophies_change, opp_id))
        conn.commit()

        if stars == 0:
            end_message = f"انتهت المعركة! نجوم: **0** ⭐\nخسرت **{-trophies_change}** كؤوس لعدم الحصول على نجمة! 😔\nغنائم: **{loot}** ذهب/إكسير. 🎁"
        else:
            end_message = f"انتهت المعركة! نجوم: **{stars}** ⭐✨\nكؤوس مكتسبة: **{trophies_change}** 🏆\nغنائم: **{loot}** ذهب/إكسير. 🎁"
        bot.send_message(call.message.chat.id, end_message, parse_mode='Markdown')
        time.sleep(0.1)

        # إشعار للخصم
        try:
            if destruction >= 50:
                bot.send_message(opp_id, f"🛑 **تعرضت للهجوم من {call.from_user.username or f'User{user_id}'}!** خسرت **{-trophies_change}** كؤوس و**{loot}** موارد. 😔", parse_mode='Markdown')
            else:
                bot.send_message(opp_id, f"✅ **تعرضت للهجوم لكن دفاعاتك صدت العدو!** ربحت **{-trophies_change}** كؤوس. 💪", parse_mode='Markdown')
        except telebot.apihelper.ApiTelegramException as e:
            if e.error_code == 403:
        ...
        # تنظيف حالة المعركة
        battle_status.pop(user_id, None)

    threading.Thread(target=battle_thread).start()
    c.execute("UPDATE players SET last_activity = ? WHERE id=?", (datetime.now().isoformat(), user_id))
    conn.commit()
    time.sleep(0.1)
    bot.answer_callback_query(call.id, "بدأ الهجوم!")
...
def battle_actions(call):
    ...
    if data[0] == 'withdraw':
        user_id = int(data[1])
        opp_id = int(data[2])
        destruction = int(data[3])
        
        # التحقق من حالة المعركة
        if not battle_status.get(user_id, {}).get('running', False):
            bot.answer_callback_query(call.id, "المعركة انتهت بالفعل أو لم تبدأ!", show_alert=True)
            return
        
        # إيقاف المعركة
        battle_status[user_id]['running'] = False
        
        c.execute("SELECT level, troops, trophies FROM players WHERE id=?", (user_id,))
        player_row = c.fetchone()
        if not player_row:
            bot.answer_callback_query(call.id, "حدث خطأ في بياناتك!")
            return
        level, troops_str, current_trophies = player_row
        ...
        loot_percent = 0.1 + 0.05 * stars
        loot = int(loot_percent * 1000 * level * 0.5)  # غنائم جزئية

        loss_percent = 0.5 if stars < 2 else 0.3
        new_troops = lose_troops(troops_str, loss_percent)

        c.execute("UPDATE players SET trophies = trophies + ?, gold = gold + ?, elixir = elixir + ?, troops = ? WHERE id=?", (trophies_change, loot, loot, new_troops, user_id))
        c.execute("UPDATE players SET trophies = trophies - ? WHERE id=?", (trophies_change, opp_id))
        conn.commit()

        if stars == 0:
            end_message = f"انسحبت من المعركة! نجوم: **0** ⭐\nخسرت **{-trophies_change}** كؤوس لعدم الحصول على نجمة! 😔\nغنائم جزئية: **{loot}** ذهب/إكسير."
        else:
            end_message = f"انسحبت من المعركة! نجوم: **{stars}** ⭐✨\nكؤوس مكتسبة: **{trophies_change}** 🏆\nغنائم جزئية: **{loot}** ذهب/إكسير."
        bot.send_message(call.message.chat.id, end_message, parse_mode='Markdown')
        bot.answer_callback_query(call.id, "انسحبت بنجاح!")

        # محاولة تعديل رسالة بداية المعركة لإزالة الأزرار
        try:
            bot.edit_message_text(f"تم الانسحاب من المعركة ضد **{opp_username}**!", 
                                 chat_id=battle_status[user_id]['chat_id'], 
                                 message_id=battle_status[user_id]['message_id'], 
                                 parse_mode='Markdown')
        except:
            pass  # قد تكون الرسالة غير موجودة أو تم تعديلها

        # إشعار للخصم
        try:
            c.execute("SELECT username FROM players WHERE id=?", (user_id,))
            attacker_username = c.fetchone()[0] or f"User{user_id}"
            if destruction >= 50:
                bot.send_message(opp_id, f"🛑 **تعرضت للهجوم من {attacker_username}!** خسرت **{-trophies_change}** كؤوس و**{loot}** موارد. 😔", parse_mode='Markdown')
            else:
                bot.send_message(opp_id, f"✅ **تعرضت للهجوم لكن دفاعاتك صدت العدو!** ربحت **{-trophies_change}** كؤوس. 💪", parse_mode='Markdown')
        except telebot.apihelper.ApiTelegramException as e:
            if e.error_code == 403:
    ...
    elif data[0] == 'next_opponent':
        user_id = int(data[1])
        bot.edit_message_text("جاري البحث عن خصم جديد...", chat_id=call.message.chat.id, message_id=call.message.message_id)
        c.execute("SELECT id, username, level, troops, gold, elixir FROM players WHERE id != ? AND id NOT IN (SELECT id FROM banned) ORDER BY RANDOM() LIMIT 1", (user_id,))
        opponent_row = c.fetchone()
        if not opponent_row:
            bot.edit_message_text("لا يوجد خصوم متاحين حالياً! 🔍", chat_id=call.message.chat.id, message_id=call.message.message_id, parse_mode='Markdown')
            bot.answer_callback_query(call.id, "لا يوجد خصوم!")
            return
        opp_id, opp_username, opp_level, opp_troops, opp_gold, opp_elixir = opponent_row
        c.execute("SELECT level FROM players WHERE id=?", (user_id,))
        level = c.fetchone()[0]
        if abs(level - opp_level) > 1:
            bot.edit_message_text("لا يوجد خصم بنفس المستوى! جرب لاحقاً. ⏳", chat_id=call.message.chat.id, message_id=call.message.message_id, parse_mode='Markdown')
            bot.answer_callback_query(call.id, "لا يوجد خصم مناسب!")
            return
        show_opponent_info(call.message.chat.id, opp_id, opp_username, opp_level, opp_troops, opp_gold, opp_elixir, user_id)
