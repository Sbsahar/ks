...
def start_attack(call):
    ...
    c.execute("SELECT level, troops, clan_name, trophies FROM players WHERE id=?", (user_id,))  # إضافة: جلب trophies للتحقق من الخسارة
    player_row = c.fetchone()
    if not player_row:
    ...
    if clan_name:
        ...
        if clan_row:
            ...
            if clan_level >= 5:
                # +1 مستوى للجنود الدعم (زيادة قوة بنسبة 10% لكل مستوى فوق 4)
    ...
    def battle_thread():
        ...
        for minute in range(5):
            ...
            effective_damage = max(0, damage - defender_power * random.uniform(0.5, 1.0))  # إصلاح: مراعاة الدفاع لجعلها واقعية
            defender_hp -= effective_damage
            if defender_hp <= 0:
                destruction = 100
            else:
                destruction = min(100, destruction + int((effective_damage / calculate_village_hp(opp_troops, opp_level)) * 100 * random.uniform(0.8, 1.2)))
            markup = InlineKeyboardMarkup()
            if destruction < 50:
        ...
        trophy_offer = 30
        if stars == 0:
            trophies_change = -10  # خسارة للمهاجم
            if current_trophies < 10:
                trophies_change = -current_trophies  # لا تذهب تحت 0
        else:
            trophies_change = stars * (trophy_offer // 3)  # +10, +20, +30

        loot_percent = 0.1 + 0.05 * stars
        loot = int(loot_percent * 1000 * level)

        # تحديث كؤوس: مهاجم + trophies_change, خصم - trophies_change (مع عكس لـ 0 نجوم)
        ...
        try:
            if destruction >= 50:
                bot.send_message(opp_id, f"🛑 **تعرضت للهجوم من {call.from_user.username}!** خسرت **{-trophies_change}** كؤوس و**{loot}** موارد. 😔", parse_mode='Markdown')
            else:
...
def battle_actions(call):
    ...
    if data[0] == 'withdraw':
        ...
        loot_percent = 0.1 + 0.05 * stars
        loot = int(loot_percent * 1000 * level * 0.5)  # غنائم جزئية عند الانسحاب (50% فقط)
        ...
        user_id = int(data[1])
        # إعادة استدعاء start_battle للبحث عن خصم جديد (لكن بدون الرسالة الأصلية)
        # يمكنك استدعاء دالة مشابهة أو إعادة تشغيل المنطق
        bot.edit_message_text("جاري البحث عن خصم جديد...", chat_id=call.message.chat.id, message_id=call.message.message_id)
        start_battle(call.message)  # أو قم بتعديل لتجنب لوب غير مرغوب
