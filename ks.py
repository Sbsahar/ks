...
def show_opponent_info(chat_id, opp_id, opp_username, opp_level, opp_troops, opp_gold, opp_elixir, user_id, is_revenge=False):
    ...
    bot.send_message(
        chat_id,
        f"خصم محتمل: <a href='tg://user?id={opp_id}'>{opp_username}</a>\n"
        f"مستوى: {opp_level}\n"
        f"جنود: {troops}\n"
        f"موارد متاحة للسرقة: {opp_gold // 10} ذهب، {opp_elixir // 10} إكسير",
        parse_mode='HTML',
        reply_markup=markup
    )
...
def start_attack(call):
    ...
    bot.send_message(
        ...
        f"بدأت معركة ضد <a href='tg://user?id={opp_id}'>{opp_username}</a> مستوى **{opp_level}**! ⚔️💥\nمدة: 5 دقائق. انتظر التحديثات.",
        parse_mode='HTML'
    )
    bot.answer_callback_query(call.id)
    ...
    def battle_thread():
        ...
        for minute in range(5):
            ...
            markup = InlineKeyboardMarkup()
            if destruction < 50:
                markup.add(InlineKeyboardButton("انسحاب 🏃", callback_data=f"withdraw:{user_id}:{opp_id}"))
            bot.send_message(
                call.message.chat.id,
                f"دقيقة **{minute+1}**: دمرت **{destruction}%** من الخصم! 🔥💥",
                reply_markup=markup,
                parse_mode='Markdown'
            )
            if destruction >= 100:
        ...
        try:
            ...
            bot.send_message(
                opp_id,
                f"🛑 **تعرضت للهجوم من <a href='tg://user?id={user_id}'>{call.from_user.first_name}</a>!** خسرت **{trophies_won}** كؤوس و**{loot}** موارد. هل تريد الانتقام؟",
                parse_mode='HTML',
...
def battle_actions(call):
    ...
    if action == "withdraw":
        opp_id = int(parts[2])
        c.execute("UPDATE players SET trophies = trophies - 10 WHERE id=?", (user_id,))
        conn.commit()
        bot.send_message(call.message.chat.id, "انسحبت من المعركة! خسرت **10** كؤوس. 🏃", parse_mode='Markdown')
        bot.answer_callback_query(call.id)
