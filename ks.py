...
c.execute('''CREATE TABLE IF NOT EXISTS players 
             (id INTEGER PRIMARY KEY, username TEXT, level INT DEFAULT 1, gold INT DEFAULT 1000, elixir INT DEFAULT 1000, 
              troops TEXT DEFAULT '{}', trophies INT DEFAULT 0, last_collect TEXT, last_activity TEXT, clan_name TEXT DEFAULT NULL)''')
    ...
    while True:
        ...
        for user_id, last_collect_str, last_activity_str in players:
            ...
            try:
                ...
                pass  # تجاهل أخطاء أخرى
            time.sleep(0.05)  # تأخير صغير لتجنب الحمل
...
def create_village(message):
    ...
    if c.fetchone():
        ...
        c.execute("INSERT INTO players (id, username, last_collect, last_activity, troops) VALUES (?, ?, ?, ?, '{}')", (user_id, username, now, now))
        conn.commit()
...
def start_battle(message):
    if not is_subscribed(message.from_user.id):
        ...
        return
    user_id = message.from_user.id
    c.execute("SELECT id, username, level, troops, trophies, clan_name FROM players WHERE id != ? AND id NOT IN (SELECT id FROM banned) ORDER BY RANDOM() LIMIT 1", (user_id,))
    opponent_row = c.fetchone()
    if not opponent_row:
        bot.reply_to(message, "لا يوجد خصوم متاحين حالياً! 🔍", parse_mode='Markdown')
        time.sleep(0.1)
        return
    opp_id, opp_username, opp_level, opp_troops, opp_trophies, opp_clan = opponent_row
    ...
    bot.reply_to(message, f"بدأت معركة ضد **{opp_username}** مستوى **{opp_level}**! ⚔️💥\nمدة: 5 دقائق. انتظر التحديثات.", parse_mode='Markdown')
    time.sleep(0.1)
    ...
    if clan_name:
        ...
        if clan_row:
            ...
            if clan_level >= 5:
                # +1 مستوى للجنود الدعم
                for troop in support_troops:
                    # تطبيق الزيادة في الحساب
                    pass
    ...
    def battle_thread():
        ...
        for minute in range(5):
            ...
            defender_hp -= damage
            destruction = min(100, int((damage / defender_hp) * 100 + destruction))
        ...
        loot = int(loot_percent * 1000 * level)  # تقريبي بناءً على مستوى

        c.execute("UPDATE players SET trophies = trophies + ?, gold = gold + ?, elixir = elixir + ? WHERE id=?", (trophies_won, loot, loot, user_id))
        ...
        try:
            if destruction >= 50:
                bot.send_message(opp_id, f"🛑 **تعرضت للهجوم من {message.from_user.username}!** خسرت **{trophies_won}** كؤوس و**{loot}** موارد. 😔", parse_mode='Markdown')
            else:
                bot.send_message(opp_id, f"✅ **تعرضت للهجوم لكن دفاعاتك صدت العدو!** ربحت **{trophies_won}** كؤوس. 💪", parse_mode='Markdown')
        except telebot.apihelper.ApiTelegramException as e:
            if e.error_code == 403:
...
- استمتع باللعب! ⚔️🏰
    ...
    buttons = [
        InlineKeyboardButton("إنشاء قرية", callback_data="copy:إنشاء قرية"),
        InlineKeyboardButton("معلوماتي", callback_data="copy:معلوماتي"),
        InlineKeyboardButton("تجميع موارد", callback_data="copy:تجميع موارد"),
        InlineKeyboardButton("ترقية قرية", callback_data="copy:ترقية قرية"),
        InlineKeyboardButton("تدريب جنود", callback_data="copy:تدريب جنود بربري 10"),
        InlineKeyboardButton("جنودي", callback_data="copy:جنودي"),
        InlineKeyboardButton("هجوم كلانس", callback_data="copy:هجوم كلانس"),
        InlineKeyboardButton("إنشاء تحالف", callback_data="copy:إنشاء تحالف اسمي"),
        InlineKeyboardButton("انضم تحالف", callback_data="copy:انضم تحالف اسم"),
        InlineKeyboardButton("دعم تحالف", callback_data="copy:دعم تحالف ذهب 1000"),
        InlineKeyboardButton("تحويل", callback_data="copy:تحويل 123456 ذهب 1000"),
        InlineKeyboardButton("توب لاعبين", callback_data="copy:توب لاعبين"),
        InlineKeyboardButton("توب تحالفات", callback_data="copy:توب تحالفات")
    ]
    markup.add(*buttons)
...
def copy_callback(call):
    ...
    bot.answer_callback_query(call.id, "تم نسخ الأمر! الصقه في الدردشة.", show_alert=True)
    # Telegram لا يدعم نسخ مباشر، لكن يمكن عرض النص
    bot.send_message(call.message.chat.id, f"الأمر: **{command}** 📋", parse_mode='Markdown')
    time.sleep(0.1)
