...
def command_callback(call):
    ...
    if command == "إنشاء قرية":
        ...
        if abs(level - opp_level) > 1:
            ...
            return
        bot.send_message(call.message.chat.id, f"بدأت معركة ضد **{opp_username}** مستوى **{opp_level}**! ⚔️💥\nمدة: 5 دقائق. انتظر التحديثات.", parse_mode='Markdown')
        def battle_thread():
            attacker_power = calculate_army_power(troops_str, level)
            defender_power = calculate_army_power(opp_troops, opp_level)
            defender_hp = calculate_village_hp(opp_troops, opp_level)
            ...
            c.execute("UPDATE players SET trophies = trophies + ?, gold = gold + ?, elixir = elixir + ? WHERE id=?", 
                      (trophies_won, loot, loot, user_id))
            c.execute("UPDATE players SET trophies = trophies - ? WHERE id=?", (trophies_won, opp_id))
            conn.commit()
