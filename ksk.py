...
c.execute('''CREATE TABLE IF NOT EXISTS clans 
             (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE, level INT DEFAULT 1, resources TEXT DEFAULT '{}', members TEXT DEFAULT '[]', 
              troop_storage TEXT DEFAULT '{}')''')  # تعديل: إضافة id كـ PRIMARY KEY AUTOINCREMENT، name UNIQUE
...
def create_clan(message):
    ...
    if row and row[0] >= cost_gold and row[1] >= cost_elixir:
        c.execute("INSERT INTO clans (name, resources, troop_storage) VALUES (?, '{}', '{}')", (clan_name,))  # تعديل: إنشاء مع id تلقائي
        clan_id = c.lastrowid  # الحصول على الـ ID الفريد
        members = [user_id]
        c.execute("UPDATE clans SET members = ? WHERE id=?", (json.dumps(members), clan_id))
        c.execute("UPDATE players SET gold = gold - ?, elixir = elixir - ?, clan_name = ? WHERE id=?", (cost_gold, cost_elixir, clan_name, user_id))
        conn.commit()
        bot.reply_to(message, f"تم إنشاء تحالف **{clan_name}** مع ID: **{clan_id}**! 🛡️🎉", parse_mode='Markdown')
    else:
...
def join_clan(message):
    ...
    if not clan_name:
        ...
        time.sleep(0.1)
        return
    c.execute("SELECT id, members FROM clans WHERE name=?", (clan_name,))
    row = c.fetchone()
    if row:
        clan_id, members_str = row
        ...
        conn.commit()
        bot.reply_to(message, f"انضممت إلى **{clan_name}** (ID: **{clan_id}**) ! 👥🎊", parse_mode='Markdown')
    else:
...
def transfer(message):
    ...
    if user_id == target_id:
        bot.reply_to(message, "لا يمكنك تحويل الموارد إلى نفسك! ⚠️", parse_mode='Markdown')
        time.sleep(0.1)
        return

    # التحقق من وجود اللاعب المرسل والمستلم في قاعدة البيانات
    c.execute("SELECT gold, elixir FROM players WHERE id=?", (user_id,))
    sender_row = c.fetchone()
    if not sender_row:
        bot.reply_to(message, "أنت غير موجود في قاعدة البيانات! أنشئ قرية أولاً. 🏘️", parse_mode='Markdown')
        time.sleep(0.1)
        return

    c.execute("SELECT gold, elixir FROM players WHERE id=?", (target_id,))
    receiver_row = c.fetchone()
    if not receiver_row:
        bot.reply_to(message, "المتلقي غير موجود في قاعدة البيانات! ⚠️", parse_mode='Markdown')
        time.sleep(0.1)
        return

    field = 'gold' if resource_type == 'ذهب' else 'elixir'
    sender_balance = sender_row[0] if field == 'gold' else sender_row[1]
    if sender_balance < amount:
        bot.reply_to(message, "موارد غير كافية! 💸", parse_mode='Markdown')
        time.sleep(0.1)
        return

    # إجراء التحويل (بدون مكافأة أو شروط تحالف)
    c.execute(f"UPDATE players SET {field} = {field} - ? WHERE id=?", (amount, user_id))
    c.execute(f"UPDATE players SET {field} = {field} + ? WHERE id=?", (amount, target_id))
    conn.commit()

    bot.reply_to(message, f"تم تحويل **{amount} {resource_type}** إلى ID **{target_id}** بنجاح! 💸🎁", parse_mode='Markdown')

    # إشعار للمستلم (اختياري، للمتعة)
    try:
        sender_username = message.from_user.username or f"User{user_id}"
        bot.send_message(target_id, f"🔔 **تلقيت {amount} {resource_type}** من **{sender_username}** (ID: {user_id})! 🎁", parse_mode='Markdown')
    except telebot.apihelper.ApiTelegramException as e:
        if e.error_code == 403:
            bot.send_message(OWNER_ID, f"لا يمكن إرسال إشعار إلى ID {target_id} لأنه حظر البوت.", parse_mode='Markdown')
...
**5. التحالفات (الكلانس)**
- **إنشاء تحالف**: تكلف **50,000** ذهب/إكسير، ويحصل على ID فريد.
...
- **تحالفي**: يعرض معلومات تحالفك (ID، مستوى، أعضاء، موارد).
...
- استمتع باللعب! ⚔️🏰
    ...
    buttons = [
        ...
        InlineKeyboardButton("تحالفي", callback_data=f"cmd:تحالفي:{user_id}"),  # إضافة زر لـ "تحالفي"
...
def command_callback(call):
    ...
    if command == "إنشاء قرية":
        ...
        bot.send_message(call.message.chat.id, "أدخل الأمر بالصيغة: **إنشاء تحالف [اسم]** 📝", parse_mode='Markdown')
    ...
    elif command == "تحالفي":  # إضافة: تنفيذ "تحالفي" عبر callback
        c.execute("SELECT clan_name FROM players WHERE id=?", (user_id,))
        row = c.fetchone()
        if row:
            clan_name = row[0]
            ...
            clan_name = None
        if not clan_name:
            bot.answer_callback_query(call.id, "أنت غير منضم إلى أي تحالف! 👥", show_alert=True)
            return
        c.execute("SELECT id, level, resources, members, troop_storage FROM clans WHERE name=?", (clan_name,))
        clan_row = c.fetchone()
        if clan_row:
            clan_id, clan_level, resources_str, members_str, troop_storage_str = clan_row
            resources = json.loads(resources_str)
            members = json.loads(members_str)
            troop_storage = json.loads(troop_storage_str)
            members_list = []
            for mem_id in members:
                c.execute("SELECT username FROM players WHERE id=?", (mem_id,))
                mem_name = c.fetchone()[0] if c.fetchone() else f"User{mem_id}"
                members_list.append(f"<a href='tg://user?id={mem_id}'>{mem_name}</a>")
            info = f"👥 **معلومات تحالفك: {clan_name}**\n" \
                   f"🆔 **ID**: {clan_id}\n" \
                   f"📊 **مستوى**: {clan_level}\n" \
                   f"💰 **موارد**: ذهب {resources.get('gold', 0)}، إكسير {resources.get('elixir', 0)}\n" \
                   f"💂 **تخزين جنود**: {troop_storage}\n" \
                   f"👤 **الأعضاء ({len(members)})**: {', '.join(members_list)}"
            bot.send_message(call.message.chat.id, info, parse_mode='HTML')
        else:
            bot.answer_callback_query(call.id, "حدث خطأ في استرجاع معلومات التحالف!", show_alert=True)
    c.execute("UPDATE players SET last_activity = ? WHERE id=?", (datetime.now().isoformat(), user_id))
    conn.commit()
...
def battle_actions(call):
    ...
    if action == "withdraw":
        ...
        opp_id, opp_username, opp_level, opp_troops, opp_gold, opp_elixir = opponent_row
        show_opponent_info(call.message.chat.id, opp_id, opp_username, opp_level, opp_troops, opp_gold, opp_elixir, user_id)
        bot.answer_callback_query(call.id)                                                                                                  @bot.message_handler(commands=['ban'])
